...
myNumbers.forEach((x,i)=>{return myNumbersIndex.push(i) })
...
let forTheLoveOfGeorge = notGeorge.map((x ,i)=>{
	return x[i] = 'George'
})
...
// Code Here
let enemies = people.filter((x, i , arr) => {
	if (!x.friend) {
		return arr[i]
	}
})
...
let totallyAwesome = people.map((x) => x.awesomeLevel).reduce((x,y) => x + y,0)
